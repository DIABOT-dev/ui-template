...
Expo Router + TypeScript template that mirrors the original Flutter dashboard/drawer/chat experience while flowing through the reusable Asinu UI Kit.
...
- Mock data in `src/demo/demoData.ts` feeds every demo screen; swap with live APIs later
- No backend/LLM integrations are included—everything is static UI driven by props
